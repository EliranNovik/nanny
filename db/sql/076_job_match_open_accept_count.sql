-- Freelancer job match: count helpers already confirmed (status available) per open request.
...
returns table (
  ...
  client_posted_requests_count bigint,
  open_accept_count bigint
)
language sql
security definer
...
as $$
  with base as (
    select
      ...
      coalesce(p.is_verified, false) as client_is_verified,
      (
        select count(*)::bigint
        from public.job_requests jposted
        where jposted.client_id = jr.client_id
      ) as client_posted_requests_count,
      (
        select count(*)::bigint
        from public.job_confirmations jc
        where jc.job_id = jr.id
          and jc.status = 'available'
      ) as open_accept_count
...
comment on function public.get_job_requests_near_location(double precision, double precision, double precision, text[], uuid, int) is
  'Freelancer matching: open job_requests near location; client verification, posted counts, and available confirmation count.';
