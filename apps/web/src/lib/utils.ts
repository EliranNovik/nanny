...
export function getNativeMapUrl(job: any, provider: 'apple' | 'google' | 'waze' = 'google') {
  if (!job) return '';
  ...
  const address = isPickupDelivery ? details.to_address : (job.location_city || job.address);
