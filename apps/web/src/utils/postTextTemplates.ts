import { requestHelpWhenLabel } from "@/lib/requestHelpWhen";
...
/** Generate display copy from structured post data using deterministic templates. */
...
export function mapProfilePostTypeToCopyType(
  postTypeId: string | null | undefined,
): PostTextPostType | null {
  switch (postTypeId) {
    case "request_help":
      return "request";
    case "offer_service":
...
function formatMetadataBudget(metadata: Record<string, unknown>): string | undefined {
  const budgetVal =
    typeof metadata.budget === "number"
      ? metadata.budget
      : typeof metadata.budget === "string" && metadata.budget.trim()
        ? Number(metadata.budget)
        : null;
  const rateVal =
    typeof metadata.rate === "number"
      ? metadata.rate
      : typeof metadata.rate === "string" && metadata.rate.trim()
        ? Number(metadata.rate)
        : null;
  const suffix = metadata.rate_type === "per_hour" ? "/hr" : "";
  if (budgetVal != null && !Number.isNaN(budgetVal)) return `₪${budgetVal}${suffix}`;
  if (rateVal != null && !Number.isNaN(rateVal)) return `₪${rateVal}${suffix}`;
  return undefined;
}

export function buildPostTextInputFromProfile(
  postTypeId: string,
  metadata: Record<string, unknown>,
  caption?: string | null,
): PostTextInput {
  const postType = mapProfilePostTypeToCopyType(postTypeId) ?? "request";
  const timeframe =
    typeof metadata.timeframe === "string" ? metadata.timeframe : undefined;
  const whenNeeded = timeframe
    ? requestHelpWhenLabel({ timeframe }) ?? humanizeSnake(timeframe)
    : undefined;
  ...
  return {
    ...
    whenNeeded,
    duration:
      typeof metadata.time_duration === "string" ? metadata.time_duration : undefined,
    budget: formatMetadataBudget(metadata),
    event_name: typeof metadata.event_name === "string" ? metadata.event_name : undefined,
    date_time: typeof metadata.date_time === "string" ? metadata.date_time : undefined,
    title: typeof metadata.title === "string" ? metadata.title : undefined,
    extraNotes: caption?.trim() || undefined,
  };
}
...
function formatJobBudget(job: Record<string, unknown>): string | undefined {
  const min =
    typeof job.budget_min === "number" ? job.budget_min : null;
  const max =
    typeof job.budget_max === "number" ? job.budget_max : null;
...
function formatJobWhen(job: Record<string, unknown>): string | undefined {
  ...
  return requestHelpWhenLabel({ timeframe }) ?? humanizeSnake(timeframe);
}
...
export function buildPostTextInputFromJob(job: Record<string, unknown>): PostTextInput {
  ...
  return {
    ...
    duration:
      typeof job.time_duration === "string" ? job.time_duration : undefined,
