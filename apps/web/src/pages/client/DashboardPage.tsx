          ...
          <Card className="border border-border/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl overflow-hidden">
            <CardContent className="p-0">
              ...
              <div className="bg-white dark:bg-zinc-900">
                ...
                {/* Job Map */}
                {(activeJob.service_type === 'pickup_delivery' || activeJob.location_city) ? (
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-slate-600 dark:text-slate-400">
                    {activeJob.start_at && (
                      <div className="flex flex-wrap items-center gap-2 col-span-2 bg-slate-100 dark:bg-zinc-800/30 px-3 py-2 rounded-xl border border-border/40 mb-1">
                        ...
                        <span className="font-bold text-slate-900 dark:text-slate-100">{new Date(activeJob.start_at).toLocaleDateString()}</span>
                        <span className="text-slate-600 dark:text-slate-400">at</span>
                        <span className="font-bold text-slate-900 dark:text-slate-100">{new Date(activeJob.start_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
                      </div>
                    )}
                    {activeJob.time_duration && (
                      <div className="flex items-center gap-2 col-span-1">
                        <Hourglass className="w-3.5 h-3.5 text-primary/70 flex-shrink-0" />
                        <span className="font-medium text-slate-900 dark:text-slate-100 text-xs">{activeJob.time_duration.replace(/_/g, "-")}</span>
                      </div>
                    )}
                    {activeJob.care_frequency && (
                      <div className="flex items-center gap-2 col-span-1">
                        <Repeat className="w-3.5 h-3.5 text-primary/70 flex-shrink-0" />
                        <span className="font-medium text-slate-900 dark:text-slate-100 capitalize text-xs">{activeJob.care_frequency.replace(/_/g, " ")}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="px-5 py-4 flex items-center gap-3 cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <p className="font-semibold text-sm text-slate-900 dark:text-slate-100">{lastConversation.otherName || "Freelancer"}</p>
                ...
                {requestsTab === "my" ? (
                  myRequests.length > 0 ? myRequests.map((req) => (
                    <div key={req.id}
                      ...
                      <Badge variant="outline" className="text-xs flex-shrink-0 border-black/10 dark:border-white/10 text-slate-600 dark:text-slate-400">
