      ...
      try {
        ...
        if (jobs.length > 0) {
          const freelancerIds = Array.from(
            new Set(
              jobs.map((j: any) => j.selected_freelancer_id).filter(Boolean),
            ),
          ) as string[];
            ...
            jobs.forEach((job: any) => {
              ...
              if (conv) {
                ...
                const fallback = fallbackConvRes.data?.find(
                  (c) => c.freelancer_id === job.selected_freelancer_id,
                );
                if (fallback) {
        ...
        const processedMessages = await Promise.all(
          (recentMessagesRes.data || []).map(async (conv: any) => {
            ...
            const otherUserId =
              conv.client_id === user.id ? conv.freelancer_id : conv.client_id;
            ...
            return {
              ...
              isUnread: lastMsg
                ? lastMsg.sender_id !== user.id && !lastMsg.read_at
                : false,
            };
          }),
        );
        setRecentMessages(
          processedMessages.filter((m) => m.lastMessage !== "No messages yet"),
        );
    ...
    return (
      <div className="min-h-screen bg-slate-50/50 dark:bg-background pb-6 md:pb-8">
        <div className="app-desktop-shell pt-8 space-y-6">
          ...
          {/* Find helpers banner */}
          <Card className="border border-slate-200/50 dark:border-white/5 shadow-sm rounded-xl">
          ...
          {/* Two-col grid: messages + requests */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            ...
            <div className="space-y-3">
              ...
              {[1, 2, 3].map((i) => (
                <Card key={i} className="border-none shadow-sm rounded-2xl">
                  <CardContent className="p-4 flex items-center gap-4">
                    <Skeleton className="h-[5.25rem] w-[5.25rem] rounded-2xl shrink-0" />
                    <div className="flex-1 space-y-2">
                      <Skeleton className="h-4 w-28" />
    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-background pb-6 md:pb-8">
      <div className="app-desktop-shell pt-8 space-y-6">
        ...
        >
          ...
          {incomingKpiCount > 0 && (
            <Badge className="absolute right-3 top-3 z-10 bg-orange-100 text-orange-600 dark:bg-orange-500/15 dark:text-orange-400 border border-orange-200/80 dark:border-orange-500/25 text-[9px] font-black h-5 px-2 rounded-md pointer-events-none" aria-hidden>
              NEW
            </Badge>
          )}
                ...
                {activeJobs.map((job) => (
                  ...
                  >
                    <DashboardLiveJobCard
                      job={job}
                      participant={{
                        full_name: job.selected_freelancer_id
                          ? freelancerProfiles[job.selected_freelancer_id]
                              ?.full_name || "Helper"
                          : "Helper",
                        photo_url: job.selected_freelancer_id
                          ? freelancerProfiles[job.selected_freelancer_id]
                              ?.photo_url || undefined
                          : undefined,
                        average_rating: job.selected_freelancer_id
                          ? freelancerProfiles[job.selected_freelancer_id]
                              ?.average_rating
                          : undefined,
                        total_ratings: job.selected_freelancer_id
                          ? freelancerProfiles[job.selected_freelancer_id]
                              ?.total_ratings
                          : undefined,
                      }}
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
              {recentMessages.length > 0 ? (
                recentMessages.map((msg) => (
                  <Card
                    key={msg.id}
                    className="border-none shadow-[0_4px_15px_rgba(0,0,0,0.02)] rounded-2xl overflow-hidden hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer group"
                    onClick={() => {
                ...
                <div className="py-10 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mb-4">Start a conversation with a helper to get assistance.</p>
                  <Button variant="outline" size="sm" onClick={() => navigate("/client/helpers")}>
                    Find a helper
                  </Button>
                </div>
              )}
          ...
          <div className="space-y-4 min-w-0">
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
              {requestsTab === "my" ? (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 ml-auto shrink-0">
                          {confirmedHelperAvatars[req.id]?.length > 0 &&
                            (() => {
                              const helpers = confirmedHelperAvatars[
                                req.id
                              ]!.slice(0, 5);
                              const totalMatched = confirmedCounts[req.id] ?? 0;
                              const overflow = Math.max(
                                0,
                                totalMatched - helpers.length,
                              );
                              return (
                                <div
                ...
                incomingInvitationsOnly.map((notif) => {
                  ...
                  return (
                    <Card
                      key={notif.id}
                      className={cn(
                        "border border-black/[0.03] dark:border-white/[0.03] shadow-[0_4px_15px_rgba(0,0,0,0.02)] rounded-2xl overflow-hidden hover:bg-slate-50 dark:hover:bg-zinc-800 transition-all cursor-pointer transition-all",
          ...
          <Card className="border-0 shadow-lg text-center py-12 bg-card">
            <CardContent>
              ...
              <h3 className="font-semibold text-lg mb-2">
                Nothing in Helping me now
              </h3>
              <p className="text-slate-600 dark:text-slate-400 mb-4">
                Post a request to get matched with a helper.
              </p>
