...
export default function PastJobDetailsPage() {
      ...
      try {
        ...
        const otherPartyId =
          jobData.client_id === user.id
            ? jobData.selected_freelancer_id
            : jobData.client_id;
        if (otherPartyId && typeof otherPartyId === "string") {
          ...
          if (profileError)
            console.warn("[PastJobDetailsPage] profile fetch:", profileError);
    ...
    return (
      <div className="grid grid-cols-1 gap-3">
        {Object.entries(details).map(([key, value]) => {
          if (key === "custom" || key.includes("lat") || key.includes("lng"))
            return null;
          return (
            <div
              key={key}
              className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-zinc-800/50 rounded-xl border border-black/5 dark:border-white/5"
            >
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-slate-50/50 dark:bg-background flex items-center justify-center p-4">
  ...
  if (!job)
    return (
      <div className="min-h-screen bg-slate-50/50 dark:bg-background flex flex-col items-center justify-center p-4">
          ...
          <div className="space-y-8">
            ...
            <Card className="rounded-3xl border border-black/5 shadow-[0_20px_50px_rgba(0,0,0,0.05)] overflow-hidden bg-card/95">
              ...
              <div className="bg-card px-6 py-4 flex items-center justify-between border-b border-black/5">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-1.5 rounded-lg bg-slate-100 ${statusColor}`}
                  >
                    {statusIcon}
                  </div>
                  <span
                    className={`font-bold text-sm tracking-wide uppercase ${statusColor}`}
                  >
                    {statusLabel}
                  </span>
                </div>
                <Badge
                  variant="outline"
                  className="bg-slate-50 border border-black/10 text-slate-700 font-bold px-3 py-1 scale-110"
                >
                  ...
                  {job.service_type === "nanny" && (
                    <Baby className="w-3.5 h-3.5 mr-2" />
                  )}
              ...
              {otherUserId && otherParty && (
                ...
                >
                  <Avatar className="h-11 w-11 shrink-0 border border-black/10">
                    <AvatarImage
                      src={otherParty.photo_url || undefined}
                ...
                <div className="relative h-96 md:h-[450px] overflow-hidden bg-slate-50">
                  ...
                  <div className="absolute inset-x-4 bottom-4 p-6 bg-card/80 backdrop-blur-xl z-20 rounded-[2.5rem] border border-border/40 shadow-2xl">
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black text-slate-500/80 uppercase tracking-[0.2em] mb-2">
                          START DATE
                        </span>
                        <div className="flex items-center gap-3 text-slate-900">
                          <Calendar className="w-5 h-5 text-orange-500" />
                          <span className="font-extrabold text-base sm:text-lg">
                            {job.confirm_starts_at
                              ? new Date(
                                  job.confirm_starts_at,
                                ).toLocaleDateString()
                              : job.start_at
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-3 text-slate-900">
                          ...
                          <span className="font-extrabold text-base sm:text-lg capitalize">
                            {job.time_duration?.replace(/_ /g, "-") || "N/A"}
                          </span>
                        </div>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="space-y-6">
                <h3 className="font-bold text-lg tracking-tight px-1">
              ...
              <div className="space-y-6">
                <h3 className="font-bold text-lg tracking-tight px-1 text-slate-500">
                  Additional Context
                </h3>
                {job.service_details?.custom ? (
                  <div className="p-4 bg-card rounded-3xl border border-black/5 shadow-sm">
                    <span className="font-bold text-slate-400 text-[10px] uppercase tracking-widest flex items-center gap-2 mb-3 underline decoration-slate-200 underline-offset-8">
                      Historical Notes
                    </span>
                    <span className="text-slate-700 text-sm font-medium whitespace-pre-wrap leading-relaxed">
                      {job.service_details.custom}
                    </span>
                  </div>
                ) : (
              ...
              {!conversationId && (
                <div className="text-center p-8 bg-slate-50 rounded-3xl border border-dashed border-black/10">
                  <p className="text-slate-400 font-bold text-sm">
                    No active conversation found for this job.
                  </p>
                </div>
              )}
            </div>
