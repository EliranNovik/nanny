...
export function DualLocationPicker({
    ...
    const getMapCenter = () => {
        ...
        if (fromValue.lat && fromValue.lng) return { lat: fromValue.lat, lng: fromValue.lng };
        if (toValue.lat && toValue.lng) return { lat: toValue.lat, lng: toValue.lng };
    ...
    const onFromLoad = useCallback((autocompleteInstance: google.maps.places.Autocomplete) => {
        setFromAutocomplete(autocompleteInstance);
    }, []);

    const onToLoad = useCallback((autocompleteInstance: google.maps.places.Autocomplete) => {
            ...
            <div className="space-y-3">
                <div className="space-y-2">
                    ...
                    <Input
                        ...
                        onChange={(e) => onFromChange({ ...fromValue, address: e.target.value })}
                    />
                </div>
                <div className="space-y-2">
                    ...
                    <Input
                        ...
                        onChange={(e) => onToChange({ ...toValue, address: e.target.value })}
                    />
                </div>
            ...
            <div className="relative">
                ...
                <div className="mt-2 p-3 bg-muted rounded-lg">
                    <p className="text-xs text-muted-foreground">
                        ...
                        <span className={activeInput === "from" ? "text-green-600 font-semibold" : "text-red-600 font-semibold"}>
                            {activeInput === "from" ? "pickup (A)" : "delivery (B)"}
                        </span>{" "}
                        input field above, then click on the map to set that location. Or search for an address using autocomplete.
                    </p>
                </div>
            </div>
        </div>
