...
export function JobDetailsModal({ 
    ...
    useEffect(() => {
        if (!isOpen || !job || !user?.id || !targetProfileId || user.id === targetProfileId) {
    ...
    const showProfileHeart =
        Boolean(heroProfileId && user?.id && user.id !== heroProfileId);
    ...
    const footerSection =
        (incomingActionMessage || (showAcceptButton && onConfirm)) ? (
                    <div
                        className="pointer-events-none absolute inset-x-0 bottom-0 z-40 flex justify-center px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-1"
                    >
                        {incomingActionMessage ? (
                            ...
                            <div className={cn(footerBarShell, "flex flex-row items-center justify-center gap-4")}>
                                <Button
                                    type="button"
                                    title="Accept"
                                    aria-label="Accept"
