        ...
        <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
            <DialogContent className="sm:max-w-md rounded-2xl border-0 shadow-2xl p-0 overflow-hidden">
                ...
                <div className="px-6 pt-8 pb-6 bg-gradient-to-br from-primary/10 via-background to-background text-center">
                    <DialogHeader>
                        <DialogTitle className="text-2xl font-bold mb-1">Rate your experience</DialogTitle>
                    ...
                    <div className="mt-5 flex flex-col items-center gap-2">
                        <Avatar className="w-20 h-20 border-4 border-background shadow-lg">
                            <AvatarImage src={reviewee.photo_url || undefined} className="object-cover" />
                        ...
                        <p className="text-xs text-muted-foreground capitalize font-medium">{revieweeRole}</p>
                    </div>
                </div>
                ...
                <div className="px-6 pb-6 space-y-5">
                    {done ? (
                        <div className="flex flex-col items-center gap-3 py-6">
                            ...
                            <p className="text-lg font-bold text-emerald-600">Review saved!</p>
                            <p className="text-sm text-muted-foreground">Marking job as completed…</p>
                        </div>
                    ) : (
                        <>
                            {/* Stars */}
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <span className={cn(
                                    ...
                                    (hovered > 0 || rating > 0) ? "opacity-100" : "opacity-0 pointer-events-none"
                                )}>
                            ...
                            <div className="space-y-1.5">
                                <label className="text-sm font-medium text-muted-foreground">
                                    Leave a comment <span className="text-xs opacity-60">(optional)</span>
                                </label>
                                <Textarea
