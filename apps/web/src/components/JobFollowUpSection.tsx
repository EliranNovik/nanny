...
export default function JobFollowUpSection({
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await apiPost(`/api/jobs/${jobId}/details`, {
  ...
  const isAnswered = () => {
    ...
    switch (serviceType) {
      case "cleaning":
        return !!initialDetails.home_size;
      case "cooking":
        return !!initialDetails.who_for;
      case "pickup_delivery":
        return !!initialDetails.weight;
      case "nanny":
  ...
  if (!hasQuestions || dismissed || submitted || isAnswered()) {
    console.log("[JobFollowUpSection] Not rendering because:", {
      hasQuestions,
      dismissed,
      submitted,
